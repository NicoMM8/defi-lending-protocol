...
async function main() {
  ...
  await pool.addMarket(await weth.getAddress(), ethers.parseUnits("0.75", 18), ethers.parseUnits("1.08", 18));
  ...
  console.log(`   ⚠️  Health Factor (underwater): ${ethers.formatUnits(hfUnderwater, 18)}`);
  console.log(`   Position is ${hfUnderwater < WAD ? "LIQUIDATABLE 🔴" : "safe 🟢"}`);

  // ── 4. First Liquidation (50% Close Factor) ──
  console.log("\n🏦 Liquidator swoops in (1st call — Close Factor 50%)...");
  ...
  // ── 5. Second Liquidation (remaining 50%) ──
  if (hfAfter1 < WAD) {
