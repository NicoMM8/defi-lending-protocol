...
async function main() {
  ...
  const profitBuffer = fee + ethers.parseUnits("12.5", 18); // Simulated arbitrage profit
  await usdc.approve(await bot.getAddress(), profitBuffer);
  console.log(`ArbitrageBot pre-funded with ${ethers.formatUnits(profitBuffer, 18)} USDC (fee + profit)`);
  ...
  const tx = await bot.executeArbitrage(await usdc.getAddress(), flashAmt, 0);
